...
class AuthControllerTokenLinkTest {
    ...
    void browserRegistrationLinkRedirectsBeforeConsumingToken() {
        ...
        assertThat(response.getHeaders().getLocation())
                .hasToString("http://192.168.1.125:3000/verify-email?token=registration-token");
    ...
    void browserEmailChangeLinkRedirectsToFriendlyUiBeforeConsumingToken() {
        ...
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.FOUND);
        assertThat(response.getHeaders().getLocation())
                .hasToString("http://192.168.1.125:3000/verify-email?token=email-change-token&mode=email-change");
