...
t_resp=$(curl -s -w "\n%{http_code}" \
    ...
    -d "{\"name\":\"Cancel Inv Corp\",\"slug\":\"$SLUG\",\"ownerEmail\":\"admin@fams.com\"}")
...
other_t=$(curl -s -w "\n%{http_code}" \
    ...
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer $ADMIN_TOKEN" \
    -d "{\"name\":\"Other Corp\",\"slug\":\"$OTHER_SLUG\",\"ownerEmail\":\"admin@fams.com\"}")
...
if [ "$reg_status" -eq 201 ]; then
    ...
    l2=$(curl -s -X POST "$BASE_URL/api/v1/auth/login" \
        -H "Content-Type: application/json" \
        -d "{\"identifier\":\"noperm.cancel.${TS}@fams.com\",\"password\":\"Regular@1234\"}")
