...
for suite in "$SCRIPT_DIR"/auth/test_*.sh "$SCRIPT_DIR"/tenant/test_*.sh "$SCRIPT_DIR"/subscription/test_*.sh "$SCRIPT_DIR"/rbac/test_*.sh "$SCRIPT_DIR"/employee/test_*.sh "$SCRIPT_DIR"/workspace/test_*.sh "$SCRIPT_DIR"/site/test_*.sh; do
    if [ -f "$suite" ]; then
