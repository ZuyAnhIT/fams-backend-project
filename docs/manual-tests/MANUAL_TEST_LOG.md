...
| 131-135 | Saved filters, export audit, tenant suspend/reactivate, tenant operational detail, enforce plan limits + export limit | — | — | ⬜ **Đã commit ở nhánh riêng, CHƯA merge vào develop** | 2026-08-19 | Đã code+test xong trong phiên làm việc trước, commit `fd56352` trên nhánh `feature/saved-filters-tenant-ops-plan-limits-131-135` — **CHƯA merge vào `develop`** (chưa có PR). Dòng này chỉ là placeholder ghi nhớ; chi tiết đầy đủ nằm trong chính commit đó (đã có sẵn `docs/manual-tests/sprint-5-feature-131-*.md` … `sprint-6-feature-135-*.md`). Bạn tự xử lý merge — xem cảnh báo ở #136-140 bên dưới |
