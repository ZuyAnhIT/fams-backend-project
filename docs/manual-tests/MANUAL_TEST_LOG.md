...
| 80 | Tự động tạo attendance summary | — | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-17 | Không có gap, chỉ khác tên field (calculated_at → updatedAt). Cơ chế "khóa" khi HR đã điều chỉnh thủ công hoạt động đúng. Toàn bộ `test_attendance_summary.sh` pass |
| 81 | Tính đi muộn | — | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-17 | **Gap đã vá (quyết định chủ dự án: thêm ân hạn 5 phút mặc định)**: thêm `Shift.graceMinutes`, snapshot trên `CheckinRecord`, sửa công thức tính muộn (trong ân hạn = không muộn; vượt ân hạn = tính đủ số phút trễ thực). Web Admin đã bổ sung input + hiển thị. Test live xác nhận đúng: trong ân hạn `late=false`; vượt ân hạn `late=true, lateMinutes=` số phút trễ thật (không trừ ân hạn) |
| 82 | Tính về sớm | — | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-17 | Đúng AC, không có gap. Nuance quan trọng: ngày còn phiên thiếu checkout thì KHÔNG tính về sớm cho cả ngày (thiết kế có chủ đích, không phải bug). Toàn bộ `test_early_leave.sh` pass |
| 83 | Tính OT | — | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-17 | Đúng bản chất, không có gap. Cơ chế khác hẳn AC gốc (không dùng standard_hours_per_day, thực tế dùng shiftEnd+lateCheckoutMinutes); có thêm cảnh báo vượt giới hạn ngày/tuần (chỉ cảnh báo, không chặn) — xác nhận đúng thiết kế, không sửa. Toàn bộ `test_ot_minutes.sh` pass |
| 84 | Phát hiện thiếu checkout | ✅ Pass | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-17 | **2 gap đã vá**: (1) tạo notification cho cả HR lẫn nhân viên (`MISSING_CHECKOUT_EMPLOYEE`/`MISSING_CHECKOUT_HR`, gửi đúng 1 lần tại thời điểm false→true, không spam khi tính lại); (2) endpoint HR adjust (`PATCH .../adjust`) giờ ghi audit `attendance_summary_adjusted` (trước đó thiếu, trong khi unlock-and-recompute đã có). Test live xác nhận cả 2 fix qua DB thật |
| 85 | Nhân viên xem bảng công ngày/tháng | — | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-17 | **Gap đã vá**: thêm lọc `siteId` cho `/me` và `/me/monthly` + chip filter site trên App (chỉ hiện khi ≥ 2 site). Test live qua UI thật xác nhận đúng. "Violation" giữ nguyên cờ dẫn xuất theo quyết định chủ dự án (không tích hợp bảng `violations` thật) |
| 86 | HR xem bảng công tổng hợp | ✅ Pass | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-17 | **Gap đã vá**: thêm lọc `status` + `sortBy`/`sortDir` (whitelist, validate 400 khi sai) cho `/monthly` + 2 dropdown mới trên Web Admin. Test live qua UI thật xác nhận request gửi đúng tham số. KHÔNG thêm lọc workspace theo quyết định chủ dự án |
| 87 | Đăng ký thiết bị nhận push | — | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-17 | Không có gap chức năng, không cần sửa code. Regression `test_fcm_devices.sh` 13/13 pass. Luồng đăng nhập/đăng xuất trên thiết bị thật cần App team tự test tay (Expo Go không cấp được push token thật) |
| 88 | Gửi push notification | — | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-17 | **3 gap đã vá**: lưu `provider_message_id`, dừng retry sớm + **vô hiệu hóa token chết ngay khi gặp lỗi `UNREGISTERED`** (trước đây retry vô hạn mãi mãi — gap nghiêm trọng nhất nhóm này). Test live xác nhận error-code plumbing đúng + KHÔNG vô hiệu hóa nhầm cho lỗi khác. Case `UNREGISTERED` thật cần App team test tay trên thiết bị thật (gỡ app rồi trigger push) |
| 89 | Danh sách thông báo trong app/web | ✅ Pass | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-17 | **Gap đã vá**: thêm `priority` thật do backend quyết định theo eventType (snapshot lúc tạo), badge "Khẩn cấp"/"Quan trọng" trên cả Web Admin và App (chỉ hiện cho high/critical). Test live xác nhận cả 2 FE. Gap "deep_link" đã cải chính từ audit gốc — thực ra đã hoạt động qua `metadata`, không cần sửa |
| 90 | Đánh dấu đã đọc | — | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-17 | Không có gap chức năng. Bổ sung test live cho luồng batch (đánh dấu nhiều thông báo được chọn lọc) — trước đây chỉ có bằng chứng qua code, nay đã xác nhận qua API thật: chỉ đánh dấu đúng các ID được chọn, không ảnh hưởng phần còn lại |
| 91 | Tạo cấu hình random check mặc định tenant | ✅ Pass | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | **Gap đã vá**: thêm audit log cho `RandomCheckConfigService` (áp dụng chung cả 6 method mutate, ảnh hưởng #91-95). Quyết định chủ dự án: giữ nguyên `checksPerShift` số cố định, không đổi min/max. Test live qua UI thật (Playwright, kết nối backend thật): tạo/sửa config qua form, audit log hiện đúng trên trang "Nhật ký audit" với diff before/after chính xác |
| 92 | Tạo cấu hình override theo site | ✅ Pass | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | Gap audit log đã vá chung với #91. Override/effective-config/site-scope đều đúng. Test live qua UI thật: tạo override từ trang chi tiết công trình, effective-config chuyển đúng từ "kế thừa mặc định" sang override ngay sau khi tạo |
| 93 | Cấu hình số lần và khung giờ check | ✅ Pass | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-22 | Gap audit log đã vá chung với #91 (2026-08-18). **Cập nhật 2026-08-22 — phát hiện qua ca hỗ trợ thật (tenant "FOFO")**: nửa sau AC "validate nằm trong ca; hiển thị cảnh báo nếu không thể sinh lịch" chưa từng được vá thật — chỉ có validate self-consistency (số lần×khoảng cách vừa khung giờ CỦA CHÍNH NÓ), chưa hề đối chiếu khung giờ với giờ ca thật của site. Khách hàng đặt khung 15:00-17:00 cho site chỉ có ca 14:25-14:35 → không giao nhau → hệ thống âm thầm không tạo kiểm tra ngẫu nhiên nào, không báo lỗi ở đâu cả. **Đã vá cả backend lẫn Web Admin**: backend chặn 400 kèm thông điệp rõ ràng khi site override không giao với ca nào; Web Admin cảnh báo live ngay trên form trước khi submit. Test live qua API + Playwright thật: cả 2 đường (chặn khi lệch, chấp nhận khi trùng) đều đúng. `test_random_check_config_site_override.sh` PASS 16/16 |
| 94 | Cấu hình mode kiểm tra | ✅ Pass | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | Gap audit log đã vá chung với #91. Quyết định chủ dự án: giữ nguyên không có threshold GPS/face riêng. Test live qua UI thật — dropdown mode hiển thị đúng 3 lựa chọn |
| 95 | Cấu hình áp dụng theo vai trò | ✅ Pass | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | Không có gap chức năng — filter role hoạt động đúng thật. Gap audit log (dùng chung service) đã vá. AC nhắc "lead" là sai ở tầng hệ thống. Test live qua UI thật: dropdown chọn role chỉ hiện đúng "worker"/"supervisor", không có "lead" |
| 96 | Tự động sinh scheduled checks đầu ca | — | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | Không có gap — AC "random số lần trong min/max" lỗi thời (khớp #91). Test live: regression 18/18 suite pass sau đợt fix #99/#100 |
| 97 | Snapshot config khi sinh check | — | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | Không có gap — snapshot bất biến, không re-fetch config sống. Test live: regression pass |
| 98 | Tạo job gửi check | — | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | Không có gap — kiến trúc Redis ZSET + poll đúng, "bull_job_id" trong AC lỗi thời. Test live: regression pass |
| 99 | Hủy scheduled check | ✅ Pass | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | **2 gap đã vá**: thêm migration V104 cho cancelledBy/At/Reason + audit log `scheduled_check_cancelled`. Test live qua UI Web Admin thật: modal hủy có ô lý do (không bắt buộc), trang chi tiết hiển thị đúng "Lúc/Người hủy/Lý do". **Bổ sung cùng ngày**: `cancelPendingByAssignment` (hủy hàng loạt khi assignment bị hủy/employee bị terminate) trước đó chỉ có log ứng dụng — theo yêu cầu người dùng đã thêm audit log + gán `cancelledBy` cho người gây cascade, live-verify cả 2 nhánh qua API+DB, regression 18/18 randomcheck + cancel-assignment + change-employee-status suites PASS |
| 100 | Gửi random check notification | — | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | **2 gap đã vá**: thêm sentAt + notificationId. **Bug tự phát hiện khi test live**: luồng HR-trigger-ngay (#108) ban đầu quên set sentAt (khác code path với poller) — đã vá ngay trong đợt. Test live qua UI xác nhận "Giờ gửi thực tế" hiển thị đúng |
| 101 | App hiển thị random check đang chờ | ✅ Pass | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | Test live qua Expo Web thật + backend thật: đếm ngược mm:ss chạy đúng, 3 mode hiển thị đúng nhãn (Vị trí GPS / GPS + khuôn mặt / GPS + khuôn mặt + người thật), check hết hạn hiển thị pill "Hết hạn" xám. Deep-link `?checkId=` mở đúng thẳng vào panel phản hồi của check đó, không qua bước trung gian |
| 102 | Phản hồi mode chỉ vị trí | ✅ Pass | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | Backend: `test_respond_check.sh` 23/23 PASS. Frontend: panel mode `location_only` chỉ hiện nút "Xác minh vị trí và gửi", không có bước chụp ảnh — xác nhận qua UI thật |
| 103 | Phản hồi mode vị trí + Face ID | ✅ Pass | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | **1 bug hạ tầng + 1 bug test-script phát hiện khi test live, cả 2 đã fix** (xem chi tiết trong file kịch bản). Sau fix: `test_check_response_face.sh` 11/11 PASS bao gồm cả nhánh async thật qua AI worker (face_verified=true, face_verify_score có giá trị). Frontend: panel hiện đúng cảnh báo "Face ID chưa sẵn sàng" cho nhân viên chưa enroll |
| 104 | Phản hồi mode vị trí + Face ID + Liveness | ✅ Pass | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | Cáo buộc "liveness dead code" của audit gốc đã lỗi thời (bản vá cũ đã đúng). **Nâng cấp liveness từ thụ động → chủ động theo quyết định người dùng**: dùng lại `FaceLivenessCamera` (quay đầu/nháy mắt, purpose=random_check, migration V105) thay cho `FacePhotoCapture` 1 ảnh tĩnh; `livenessChallengeId` nay bắt buộc, thiếu → 422. Test live: backend 15/15 PASS qua AI worker thật (face_verified=true qua khung challenge, challenge chuyển consumed, liveness_verified giữ NULL đúng thiết kế giống check-in); frontend qua Expo Web thật — panel hiện đúng UI "Xác minh người thật" + camera thật chạy Bước 1/3 |
| 105 | Từ chối phản hồi trễ | ✅ Pass | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | Backend: `test_late_response_rejection.sh` 14/14 PASS. Frontend: deep-link tới check đã hết hạn không mở panel phản hồi (tự động bỏ chọn), card hiện đúng "Hết hạn" — xác nhận đúng ranh giới với #106 |
| 106 | Tạo violation khi không phản hồi | ✅ Pass | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | **Đã vá gap notification.** `ViolationNotificationService` mới (dùng chung với #107) gửi thông báo cho cả nhân viên bị vi phạm và mọi HR/Admin giữ quyền `violations:list` trong tenant, theo đúng pattern đã có sẵn ở missing-checkout (#84). Test live qua API+DB thật: violation `no_response` tạo đúng + 2 notification (`RANDOM_CHECK_VIOLATION_EMPLOYEE`, `RANDOM_CHECK_VIOLATION_HR`) xuất hiện đúng nội dung. Regression 26/26 |
| 107 | Tạo violation khi fail random check | ✅ Pass | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | **Cáo buộc "liveness_fail dead code" đã lỗi thời (vá cùng #104).** Đã vá gap notification (chung #106, xem trên). **Cải chính thêm: "bằng chứng không nằm sẵn trong violation" — kiểm tra lại `ViolationDetailModal.tsx` phía Web Admin thì bằng chứng (GPS+link Maps, độ chính xác, geofence, Face ID, Liveness+điểm số, ảnh khuôn mặt) ĐÃ được hiển thị đầy đủ ngay trong modal chi tiết (join qua `checkResponse`, không cần tra gián tiếp) — không phải gap thực tế ở tầng UI/API, chỉ đúng ở tầng entity thô (không lưu JSON trùng lặp, thiết kế chuẩn hoá đúng)** |
| 108 | HR kích hoạt kiểm tra ngay | ✅ Pass | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | Cải chính audit gốc "không ghi audit" xác nhận lại là sai — có ghi đầy đủ. **Đã vá gap `trigger_type`:** phát hiện field `triggeredBy` (UUID) đã tồn tại sẵn trên entity (không cần migration) — chỉ cần derive `triggerType` = 'manual_hr' nếu có `triggeredBy`, 'auto' nếu null, expose qua response DTO + filter param mới `?triggerType=`. Test live qua API thật: tạo 1 check thủ công, response trả đúng `triggerType=manual_hr` |
| 109 | HR xem danh sách scheduled checks | ✅ Pass | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | **Gap `trigger_type` đã vá cùng #108** — filter `?triggerType=auto\|manual_hr` hoạt động đúng (test live: lọc `manual_hr` chỉ trả về đúng 1 check thủ công vừa tạo, lọc `auto` chỉ trả check hệ thống). Web Admin: thêm dropdown "Lọc theo loại" (Tự động/Thủ công HR), cột "Loại" đổi sang đọc từ `triggerType` thay vì suy luận qua `manualReason`. Test live qua UI thật: dropdown hiển thị đúng, 4 filter cũ (site/employee/status/ngày) không đổi |
| 110 | HR xem chi tiết random check | ✅ Pass | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | Gap "sent_at" đã tự động được vá từ #100 như dự đoán — xác nhận lại qua test live: chi tiết 1 check thủ công trả đúng `sentAt` có giá trị thật. Bổ sung thêm `triggerType` vào response chi tiết cho nhất quán với #108/#109 |
| 111 | HR override check-in | ✅ Pass | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | Xác nhận lại: gap "không ghi audit" đã tự vá từ đợt #79 trong phiên này — `checkin_overridden` ghi đầy đủ before/after. `test_override_checkin.sh` PASS trong regression 31/31, không cần sửa gì |
| 112 | HR chỉnh attendance summary | ✅ Pass | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | Xác nhận lại: gap "không ghi audit" đã tự vá từ đợt #84 trong phiên này — `attendance_summary_adjusted` ghi đầy đủ before/after + reason, bản ghi được khóa khỏi tính lại tự động. `test_adjust_attendance_summary.sh` PASS, không cần sửa gì |
| 113 | Nhân viên gửi giải trình check-in lỗi | ✅ Pass | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | **Đã vá gap notification** cho cả 2 đường giải trình: `CheckinService.explainCheckin(WithPhoto)` (gắn với checkin) và `ViolationService.explainViolation(WithPhoto)` (gắn thẳng với violation — phát hiện endpoint này ĐÃ TỒN TẠI SẴN, **cải chính "gap MỚI: không liên kết được với violation" của audit — SAI/lỗi thời**, `POST .../violations/{id}/explain` đã cho giải trình gắn trực tiếp vào violation từ trước). Test live qua API+DB thật: cả 2 đường đều tạo đúng notification cho HR |
| 114 | HR xem danh sách vi phạm | ✅ Pass | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | **Đã vá filter `affectsAttendance`** (field có sẵn, chỉ thiếu predicate) — test live: lọc true/false đều trả đúng kết quả tách biệt. Web Admin: thêm dropdown "Ảnh hưởng công". Gap `severity` giữ nguyên — field không tồn tại, cần đổi schema, ngoài phạm vi vá nhanh |
| 115 | HR xem chi tiết violation | ✅ Pass | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | **Đã vá gap thiếu bằng chứng cho violation từ check-in thường** — `ViolationDetailResponse` giờ join thêm `checkin` (GPS, geofence, Face ID, liveness, điểm khớp mặt) khi violation có `checkinId`, giống hệt cách `checkResponse` đã có cho random-check. Web Admin hiển thị đúng block "Bằng chứng check-in" mới. **Quyết định người dùng: giữ nguyên không phân tầng hiển thị theo vai trò** (ai có `violations:read` vẫn thấy đầy đủ) |
| 116 | Xác nhận vi phạm | ✅ Pass | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | Cải chính audit gốc "không refresh summary" xác nhận lại là sai — đã vá từ trước. **Đã vá gap audit log** (chung với #117/#118) — `ViolationService` giờ ghi `violation_confirmed`/`violation_dismissed`/`violation_attendance_impact_updated` qua helper `recordViolationAudit`. Test live qua API/DB thật: PASS |
| 117 | Bỏ qua vi phạm | ✅ Pass | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | **Đã vá cả 3 gap**: (1) notification cho nhân viên qua `VIOLATION_DISMISSED_EMPLOYEE`; (2) audit log (chung #116/#118); (3) **gap nghiêm trọng nhất — `dismissViolation` giờ tự động set `affectsAttendance=false` + `attendanceImpactReviewed=true`**, bảng công không còn coi vi phạm đã bỏ qua là đang ảnh hưởng nữa. Test live: affects_attendance chuyển đúng true→false, notification + audit đều đúng nội dung |
| 118 | Cập nhật ảnh hưởng công | ✅ Pass | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | Đã vá audit log (chung #116/#117). **Đã hỏi người dùng về gap "sửa được bất kỳ lúc nào kể cả violation đã dismissed" — quyết định GIỮ NGUYÊN linh hoạt** (không khóa theo AC), lý do: hữu ích khi phát hiện sai sót muộn, đã có audit log mới để truy vết. Test live: cập nhật + audit log đều đúng |
| 119 | Dashboard nhân viên | ✅ Pass | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | **Cải chính audit gốc: "thiếu random-check-pending, violation-pending, notifications" phần lớn SAI/lỗi thời** — `EmployeeDashboardResponse.alerts` đã có `pendingExplanations` (gộp checkin pending_review + violation unresolved) và `unreadNotifications`; Mobile App Home screen gọi thêm `useMyPendingRandomChecks()` riêng và hiển thị đúng badge "N yêu cầu cần phản hồi ngay" trên card Kiểm tra ngẫu nhiên — không có gap thực tế ở màn hình người dùng thấy, dù DTO endpoint dashboard đơn lẻ không gộp random-check. Không cần sửa gì |
| 120 | Dashboard HR | ✅ Pass | ✅ Pass | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | **Đã vá cả 2 gap thật.** (1) Filter site: thêm `?siteId=` cho mọi khối số liệu — attendance/checkin/violation lọc trực tiếp qua siteId có sẵn, personnel (không có siteId trên Employee) lọc qua tập employeeId từ `AssignmentRepository.findDistinctEmployeeIdsByTenantIdAndSiteIdIn`. (2) Thêm 2 metric mới: `pendingReview` (checkin pending_review) và `missingCheckoutToday`. Web Admin: thêm dropdown lọc site + 2 StatCard mới. Test live qua API thật: lọc site A/B trả đúng số liệu tách biệt theo assignment/violation |
| 121 | Dashboard giám sát công trình | ✅ Pass (Playwright thật) | 🟡 Code đã thêm, chưa test trên thiết bị/simulator | 🟡 **PARTIAL — chỉ còn thiếu Mobile App live test** | 2026-08-18 | Đã vá gap: dashboard supervisor thiếu cảnh báo random-check-pending + violation chưa xử lý theo site. **Test live qua Playwright thật** (không phải chỉ đọc code): tạo tài khoản SITE_SUPERVISOR thật, seed 1 random check pending + 1 violation chưa xử lý, xác nhận đúng 2 Tag cảnh báo "1 random check chờ phản hồi"/"1 vi phạm chưa xử lý" hiện trên site-card qua DOM thật. Mobile App đã thêm cùng UI, `tsc --noEmit` sạch nhưng chưa test trên thiết bị/simulator thật (Playwright không lái được React Native) |
| 122 | Báo cáo công ngày | ✅ Pass (Playwright thật) | — (không thuộc phạm vi Mobile) | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | **Quyết định người dùng: làm ngay filter workspace cho cả 4 endpoint Report (#122-125), không hoãn.** Đã vá + test live qua Playwright thật: chọn workspace trong dropdown mới → stat "Có mặt" và số dòng bảng giảm đúng từ 2 xuống 1, xác nhận dropdown kích hoạt fetch lọc thật |
| 123 | Báo cáo công tháng | ✅ Pass (Playwright thật) | — (không thuộc phạm vi Mobile) | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | Đã vá 2 gap: filter workspace + field `violationCount` mới (đếm TOÀN BỘ loại vi phạm, không chỉ random-check-derived). Test live qua Playwright: chọn workspace → stat "Nhân viên" và số dòng bảng giảm đúng từ 2 xuống 1; cột "Vi phạm" hiện đúng trong bảng |
| 124 | Export bảng công ra Excel | ✅ Pass (Playwright thật) | — (không thuộc phạm vi Mobile) | ✅ **PASS — ĐÃ KHÓA**, gap nghiêm trọng nhất đợt này | 2026-08-18 | Đã vá 3 gap (workspace filter, dòng TOTAL, audit log thiếu) + **2 bug runtime phát sinh khi vá**: (1) NPE do `employeeCode` null trong `Collectors.toMap`; (2) **audit log export bị Hibernate âm thầm bỏ qua** (transaction read-only của caller khiến insert không bao giờ flush, không exception, không log) — vá bằng `AuditLogService.record()` đổi sang `REQUIRES_NEW`, áp dụng chung mọi caller. **Test live qua Playwright thật**: bấm nút "Xuất Excel" qua UI 2 lần (có/không workspace), tải file .xlsx thật, parse bằng openpyxl xác nhận đúng 2 dòng (không filter) và 1 dòng (có filter) |
| 125 | Báo cáo vi phạm theo kỳ | ✅ Pass (Playwright thật) | — (không thuộc phạm vi Mobile) | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | Đã vá filter workspace. Test live qua Playwright: chọn workspace → "Tổng vi phạm"/"Chưa xử lý"/"Ảnh hưởng bảng công" đều giảm đúng từ 2 xuống 1. Gap `resolved` filter trên GET list là quyết định thiết kế có chủ đích từ #106, không sửa |
| 126 | Báo cáo hiện diện theo site | ✅ Pass (Playwright thật) | — (gián tiếp qua #121) | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | **Cải chính audit gốc SAI/lỗi thời**: đánh dấu "ĐÃ XONG" nhưng thực tế thiếu cả filter workspace lẫn số liệu violation mà AC yêu cầu rõ (cùng dạng gap #122/123/125). Đã vá cả 2 — test live qua API thật: filter workspace tách đúng nhân viên, `unresolvedViolations`/`totalUnresolvedViolations` đếm đúng. Test live qua Playwright thật: dropdown workspace mới + StatCard "Vi phạm chưa xử lý" (5 card) hiển thị đúng trên UI thật |
| 127 | Báo cáo trạng thái Face ID | ✅ Pass (Playwright thật) | — (không thuộc phạm vi Mobile) | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | Đã vá cả 3 gap: (1) **quality score** — phát hiện fams-ai tính điểm anti-spoof lúc enroll nhưng vứt bỏ hoàn toàn, không phải "chưa có dữ liệu" — đã hỏi người dùng, chọn vá ngay, thêm migration + sửa cả ai-service lẫn Java, test live xác nhận điểm chảy đúng end-to-end; (2) export not-enrolled — endpoint mới, tái dùng logic filter có sẵn; (3) thiếu `siteId` filter (audit gốc bỏ sót, chỉ có departmentId). Enrollment date xác nhận đã có sẵn từ trước, KHÔNG phải gap. Test live qua Playwright thật: dropdown site + cột "Chất lượng" hiển thị đúng, bấm "Xuất DS chưa đăng ký" tải file thật thành công |
| 128 | Tìm kiếm nhanh nhân viên/site/check-in | ✅ Pass (Playwright thật) | — (không thuộc phạm vi Mobile) | ✅ **PASS — ĐÃ KHÓA** | 2026-08-18 | Đã vá gap thiếu nhóm kết quả `violations` (đúng như audit gốc). Phát hiện thêm: access control (tôn trọng quyền truy cập) đã được vá TỪ TRƯỚC ở 1 commit riêng sau ngày audit gốc — không phải gap còn tồn đọng, chỉ thiếu test coverage khóa lại hành vi đó (ghi nhận, chưa vá trong đợt này). Test live qua API thật: search theo tên → nhóm `violations` trả đúng vi phạm chưa xử lý. Test live qua Playwright thật: ô tìm kiếm nhanh trên header hoạt động đúng, không crash |
| 129 | Thông báo lỗi thân thiện | 🟡 Code đã thêm, chưa test trên thiết bị | — | 🟡 **PARTIAL — chỉ Mobile App, chưa test thiết bị thật** | 2026-08-18 | **Cải chính audit gốc SAI hoàn toàn** ("chưa làm gì") — thực tế đã có mapping lỗi tiếng Việt khá đầy đủ từ trước cho cả check-in lẫn random-check. 2 gap thật: (1) thiếu gợi ý hành động cụ thể (di chuyển gần hơn / chụp lại ảnh) — đã thêm 2 card gợi ý có điều kiện theo nguyên nhân lỗi; (2) thiếu nút "Liên hệ HR" bấm được (trước chỉ là text tĩnh) — đã thêm nút điều hướng tới màn Trợ giúp sẵn có. `tsc --noEmit` sạch. **CẦN BẠN TEST THỦ CÔNG trên thiết bị/simulator** — Playwright không lái được React Native |
| 130 | Bản đồ site và vị trí hiện tại | ✅ Pass (Playwright thật) | 🟡 Cảnh báo GPS chưa test thiết bị | 🟡 **PARTIAL — Web Admin ĐÃ KHÓA, chỉ còn thiếu Mobile test thiết bị** | 2026-08-18 | Cải chính 1 phần audit gốc: bản đồ hiển thị vị trí/geofence trên Mobile App **đã có sẵn từ trước** (audit gốc chỉ nhắc CRUD backend, bỏ sót phần UI). 2 gap thật: (1) không cảnh báo GPS accuracy thấp — đã thêm banner cảnh báo ngưỡng 50m (khớp ngưỡng risk-score backend), **cần bạn test trên thiết bị thật**; (2) không có khái niệm "ẩn polygon theo policy" ở đâu trong data model — đã hỏi người dùng, chọn thêm cờ per-site mặc định HIỆN (`hidePolygonFromEmployee`), migration mới, xuyên suốt backend→mobile. Test live qua API thật: bật cờ → `coordinates=null` nhưng giữ `bufferMeters`. Test live qua Playwright thật: toggle "Ẩn vùng geofence khỏi nhân viên" trong form sửa site hoạt động đúng, lưu thành công |
| 131-135 | Saved filters, export audit, tenant suspend/reactivate, tenant operational detail, enforce plan limits + export limit | ✅ Pass (Playwright thật) | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-19 | **Cập nhật**: nhánh `feature/saved-filters-tenant-ops-plan-limits-131-135` (commit `fd56352`) đã được merge vào `develop` qua PR #91 — tình trạng "chưa merge" ghi trước đó đã lỗi thời, xác nhận lại qua rebuild+redeploy: `maxExportsPerMonth` và các tính năng #131-135 hoạt động đúng trên container thật. Chi tiết đầy đủ từng tính năng nằm trong `docs/manual-tests/sprint-5-feature-131-*.md` … `sprint-6-feature-135-*.md` (đã test live qua API + Playwright thật khi code xong, trước khi commit) |
| 136 | Xem danh sách audit log | ✅ Pass (Playwright thật) | — (không thuộc phạm vi Mobile) | ✅ **PASS — ĐÃ KHÓA** | 2026-08-19 | **Cải chính audit gốc 07-22 SAI/lỗi thời** — ghi "AuditLogService.record() không được gọi ở đâu" nhưng thực tế hiện gọi ~48 lần trên 25+ service. Không sửa code, chỉ audit lại + xác nhận. Test live API thật: `test_audit_logs.sh` 14/14. Test live Playwright thật: trang "Audit toàn hệ thống" hiển thị 12,409 dòng thật/621 trang, bộ lọc đầy đủ hoạt động đúng. Xem `docs/manual-tests/sprint-6-feature-136-138-audit-viewer.md` |
| 137 | Xem diff old/new value | ✅ Pass (Playwright thật) | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-19 | Cải chính audit gốc lỗi thời tương tự #136 — mask PII đã đủ `totpSecret`/`backupCodes`/`nationalId` từ trước. Không sửa code. Test live Playwright thật: click "Xem" 1 dòng UPDATE Employee → modal diff hiển thị đúng `status: active → inactive`, kèm request ID/IP/user agent |
| 138 | Trace theo request_id | ✅ Pass (Playwright thật) | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-19 | Cải chính audit gốc lỗi thời tương tự #136. Không sửa code. Test live: 1 request thật sinh nhiều dòng audit cùng request_id, trace trả đúng timeline. **Gap nhỏ CHƯA vá, ghi nhận**: AC nhắc "show metadata endpoint/status nếu có" — entity không có 2 cột này, cần migration + populate ~48 call site nếu muốn thêm, không tự ý mở rộng phạm vi |
| 139 | Quản lý template thông báo | ✅ Pass (Playwright thật) | — (không thuộc phạm vi Mobile) | ✅ **PASS — ĐÃ KHÓA** | 2026-08-19 | Cải chính audit gốc SAI hoàn toàn — ghi "CRUD thuần túy không có tác dụng" nhưng thực tế `NotificationService.createNotification` đã gọi `renderTemplateIfExists` làm chốt chặn duy nhất mọi notification từ trước. Không sửa code. Test live API thật: `test_notification_templates.sh` 29/29. Test live Playwright thật: tạo template qua UI → toast xác nhận → list cập nhật ngay, đã xóa dọn sau test |
| 140 | Retry và fallback notification | — (không có FE thay đổi trực tiếp) | 🟡 Cần bạn test badge "Khẩn cấp" trên thiết bị/app thật | 🟡 **PARTIAL — Backend ĐÃ KHÓA (đã merge), còn thiếu 1 case UI thật** | 2026-08-19 | Retry+backoff đã đúng từ trước. **Đã vá gap thật**: fallback trước đây chạy vô điều kiện cho MỌI loại — nay chỉ chạy khi priority="critical" đúng AC. Nâng `RANDOM_CHECK_SENT` từ "high" lên "critical" (quyết định cùng người dùng). Test live API+DB thật xác nhận đúng thiết kế. `test_fcm_retry_fallback.sh` PASS 11/11. **Cập nhật 2026-08-19 (sau đó)**: nhánh `feature/saved-filters-tenant-ops-plan-limits-131-135` (#131-135) VÀ `feature/audit-notification-template-fallback-136-140` (#136-140) đã được merge vào `develop` (PR #91, #92) — tình trạng "container thiếu #131-135" đã được giải quyết, đã rebuild+redeploy xác nhận đầy đủ cả 2 đợt cùng chạy đúng (`maxExportsPerMonth` trả về bình thường). Còn thiếu: badge "Khẩn cấp" chưa test trực quan trên UI thật (cần 1 luồng random-check thật đủ điều kiện, chưa dựng được trong phiên này). Xem `docs/manual-tests/sprint-6-feature-139-140-notification-templates-fallback.md`. **Cập nhật 2026-08-22 (follow-up, phát hiện qua ca hỗ trợ FOFO thật)**: bug thật — nhân viên **chưa từng đăng ký thiết bị nào** (0 dòng `user_devices`, chưa mở app lần nào) bị bỏ qua fallback hoàn toàn (early-return trước khi kiểm tra `fallbackEligible`), im lặng không nhận cả push lẫn email dù eventType là critical. Đã vá `UserDeviceService#sendPush` — nhánh 0-thiết-bị nay cũng gọi `sendEmailFallback` khi `fallbackEligible=true`. Rebuild+redeploy+test live xác nhận `notification_delivery_logs` nay có `EMAIL_FALLBACK|FALLBACK_EMAIL_SENT`. Thêm case test mới (mục 7) vào `test_fcm_retry_fallback.sh`, PASS 15/15. Regression 4 file liên quan PASS đủ (`test_fcm_retry_fallback.sh` 15/15, `test_fcm_devices.sh` 13/13, `test_manual_check.sh` 16/16, `test_dispatch_notification.sh` 9/9), `run_all.sh` (146 suite) đã chạy lại xác nhận không ảnh hưởng diện rộng. Xem `docs/manual-tests/sprint-6-feature-140-zero-device-fallback.md` |
| 141 | Cấu hình nhận thông báo cá nhân | ✅ tsc sạch (chưa test UI trực quan) | ✅ tsc sạch (chưa test UI trực quan) | 🟡 **PARTIAL — Backend ĐÃ KHÓA, cần bạn test UI thật** | 2026-08-19 | Audit gốc đúng: gap có thật. **Đã vá**: chặn tắt in-app/push cho eventType priority=critical (gate theo priority, không hardcode eventType) — trả 422 `MANDATORY_NOTIFICATION`. Thêm field `mandatory` vào response để FE khóa switch mà không cần hardcode danh sách. Đã wire cả Web Admin lẫn Mobile App (khóa Switch + icon khóa), `tsc --noEmit` sạch cả 2 repo. Test live qua API thật: PUT tắt RANDOM_CHECK_SENT → 422; PUT tắt EMPLOYEE_INVITED → vẫn 200. `test_notification_settings.sh` PASS 29/29. **CẦN BẠN TEST THỦ CÔNG**: xác nhận trực quan trên UI thật (Web Admin + Mobile) rằng switch "Kiểm tra ngẫu nhiên" hiển thị khóa và không bấm tắt được — chưa chạy Playwright cho phần này. Xem `docs/manual-tests/sprint-6-feature-141-144-145-146.md` |
| 144 | Dọn dữ liệu ảnh và notification cũ | — (không có UI cho field mới trong đợt này) | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-19 | Audit gốc nửa đúng: phần "không xóa ảnh selfie" SAI (đã xóa từ trước qua fams-ai), phần "không per-tenant" ĐÚNG. **Đã vá phần per-tenant**: migration thêm `tenant_settings.data_retention_days`, viết lại `DataRetentionJob` loop từng tenant áp dụng đúng retention riêng cho cả notification lẫn ảnh selfie (sửa cả ai-service Python để quét đúng thư mục theo tenant). Delivery logs giữ global-only có lý do rõ ràng (không có tenant_id, không per-tenant hoá được nếu không đổi schema). Test live qua API + ai-service + DB thật (không có endpoint trigger job thủ công, test từng thành phần trực tiếp): xác nhận file/notification của tenant khác KHÔNG bị đụng tới khi sweep 1 tenant. `test_data_retention.sh` (mới) PASS 7/7; `test_tenant_settings.sh` PASS 16/16; regression `test_checkin_face.sh` 5/5 sau rebuild fams-ai |
| 145 | Mask dữ liệu nhạy cảm trong audit và API | — (không sửa code) | — (không sửa code) | ✅ **PASS — ĐÃ KHÓA** | 2026-08-19 | **Cải chính audit gốc SAI** — nationalId đã `@Masked` từ trước trên EmployeeResponse/EmployeeDetailResponse, chỉ thiếu test coverage. Không sửa code, chỉ thêm 3 case test cho nationalId. Test live qua API thật với data thật: HR thấy `nationalId:"***"`, TENANT_ADMIN/PLATFORM_ADMIN thấy đầy đủ. `test_data_masking.sh` PASS 6/6 |
| 146 | Guard quyền API theo RBAC | — (không có UI riêng cho denial) | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-19 | Audit gốc đúng: gap có thật. **Đã vá**: `GlobalExceptionHandler.handleAccessDenied` (chốt chặn duy nhất cho mọi AccessDeniedException, 116 chỗ @PreAuthorize đều qua đây) nay ghi audit `ACCESS_DENIED` best-effort, theo khuôn mẫu `PLAN_LIMIT_DENIED`. Test live qua API + DB thật: user không có role trên tenant PATCH profile → 403 đúng + có đúng dòng audit `ACCESS_DENIED` với tenant_id/path/lý do. `test_permission_guards.sh` thêm case tự-setup, PASS 16/16 |
| 148 | Kịch bản kiểm thử end-to-end | N/A (backend + tài liệu) | N/A | ✅ **PASS — ĐÃ KHÓA** | 2026-08-19 | **Phát hiện việc đã xong từ đợt audit 2026-08-06 (nhánh đã merge) nhưng BACKLOG.md chưa cập nhật checkbox** — mục B.8 trong `manual-test-scenarios.md` (luồng 15 bước tenant→...→export) đã có sẵn. Audit lại kỹ AC phát hiện **gap thật còn sót**: B.8 thiếu hẳn đoạn "random check → violation" dù AC liệt kê rõ. Đã vá: thêm 2 bước vào B.8, VÀ viết mới `tests/e2e_uat_go_live_flow.sh` — biến tài liệu thành 1 script tự động hoá chạy được thật, nối đúng 1 chuỗi tenant→site→shift→employee→assignment→checkin/checkout→recompute→random check→vi phạm→HR xử lý→báo cáo→export. Test live qua API thật, chạy 2 lần liên tiếp: PASS 16/16 cả 2 lần. Phát hiện phụ có giá trị: script chứng minh payroll-readiness guard (đợt #124) chặn đúng export (409) khi có random-check thất bại trong tháng — 2 tính năng độc lập liên kết đúng nghiệp vụ |
| 149 | Hướng dẫn Admin/HR/Employee | N/A (tài liệu) | N/A | ✅ **PASS — ĐÃ KHÓA** | 2026-08-19 | Tương tự #148 — đã viết xong từ 2026-08-06 (`docs/user-guides/huong-dan-su-dung-theo-vai-tro.md`, 163 dòng), chỉ chưa cập nhật checkbox BACKLOG.md. Đối chiếu lại với 3 quy tắc nghiệp vụ mới thêm SAU ngày viết tài liệu (mandatory notification #141, retention theo tenant #144, audit khi bị từ chối quyền #146) — phát hiện tài liệu bị lỗi thời ở 3 điểm, đã vá đúng 3 mục liên quan, không viết lại toàn bộ |
| 150 | Checklist triển khai tenant đầu tiên | N/A (tài liệu) | N/A | ✅ **PASS — ĐÃ KHÓA** | 2026-08-19 | Tương tự #148/#149 — đã viết xong từ 2026-08-06 (`docs/deployment/go-live-checklist.md`, 8 mục, dẫn nguồn từ sự cố thật), chỉ chưa cập nhật checkbox BACKLOG.md. Đã vá 2 điểm lỗi thời (field `mandatory` mới trên notification-event-types, `dataRetentionDays` riêng theo tenant) + cập nhật câu dẫn tới B.8 cho khớp bản B.8 mới có thêm bước random check/violation |
| 5,8,9,10,11,16,138,144 | Đợt vá "gap còn tồn đọng" (audit log, /auth/me, tenant list, trace endpoint/status, delivery_logs theo tenant) | ✅ Pass (API thật) | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-19 | Người dùng yêu cầu vá toàn bộ danh sách gap đã tổng kết. Kết quả: **4/7 gap hóa ra KHÔNG còn tồn tại** — audit log `LOGOUT_ALL`/`RESET_PASSWORD`/`CHANGE_PASSWORD`/`UPDATE_PROFILE` (#5,#8,#9,#11) đã được ghi từ trước (code đã commit sẵn, chỉ ghi chú BACKLOG.md lỗi thời), xác nhận lại qua API thật + query `audit_logs`. **3 gap thật đã vá**: (1) `#10` — thêm `totpEnabled`/`currentTenantId`/`currentTenantName`/`currentTenantRole` vào `GET /auth/me`; (2) `#16` — thêm `planName`/`planId`/`subscriptionStatus` vào `GET /tenants` list (batch query, không N+1); (3) `#138` — thêm `endpoint` (tự động lấy trong `record()`) và `httpStatus` (backfill sau qua `RequestIdFilter`, chỉ chạy khi request có ghi audit) vào audit log; (4) `#144` đợt 2 — thêm `tenant_id` cho `notification_delivery_logs` (migration V109), per-tenant hoá luôn phần dọn dẹp còn sót. **SMS fallback (#140)**: đã hỏi người dùng, quyết định bỏ qua đợt này (cần tài khoản nhà cung cấp SMS thật mới test sống được). **13 checkbox tài liệu cũ** (#2,5,8-13,15-19): đối chiếu MANUAL_TEST_LOG xác nhận đều đã pass từ lâu, tick lại cho khớp. Test live qua API thật cho cả 4 fix, có bằng chứng cụ thể trong `docs/BACKLOG.md`. Regression: test_profile.sh 3/3, test_profile_fields_and_avatar.sh 8/8, test_update_profile.sh 15/15, test_logout_all.sh 6/6, test_change_password.sh 10/10, test_forgot_reset_password.sh 9/9, test_list_tenants.sh 9/9, test_create_tenant.sh 12/12, test_audit_logs.sh 14/14, test_fcm_devices.sh 13/13, test_fcm_retry_fallback.sh 11/11, test_data_retention.sh 7/7, test_permission_guards.sh 16/16 — toàn bộ PASS. Chạy thêm `run_all.sh` (toàn bộ 146 test suite tự động của dự án) sau khi rebuild+redeploy — **146/146 PASS, 0 FAIL**, xác nhận không hồi quy trên diện rộng |
| 131 | Lưu bộ lọc thường dùng | ✅ Pass (Playwright thật) | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-19 | **Cải chính audit gốc HOÀN TOÀN LỖI THỜI** ("chưa làm gì") — thực tế đã xây đầy đủ từ 2026-08-06 (backend module `savedfilter` riêng + FE component `SavedFilterToolbar`), chỉ mới tích hợp ở đúng 1 màn (Vi phạm). Đã wiring thêm vào trang Nhân viên + Công trình. Test live qua Playwright thật cả 2 trang: lưu bộ lọc → toast xác nhận → reload → chọn lại từ dropdown → filter áp dụng đúng → xóa → dropdown rỗng đúng |
| 132 | Export danh sách vi phạm | — (không có FE thay đổi) | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-19 | Đã vá gap thiếu audit log (đúng như audit gốc, không lỗi thời — khác #124, đây là thiếu hoàn toàn chứ không phải bug transaction âm thầm). Test live qua API thật: export → query DB thấy đúng dòng `EXPORT_VIOLATIONS`. Photo-reference AC xác nhận không phải gap (export không xuất cột ảnh nào) |
| 133 | Khóa/mở tenant | — (không có FE thay đổi) | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-19 | **Cải chính audit gốc SAI 1 phần**: "không ghi audit" sai (đã có từ trước, dùng REQUIRES_NEW an toàn) — chỉ "không gửi notification" đúng. Đã vá: notification `TENANT_SUSPENDED_OWNER`/`TENANT_REACTIVATED_OWNER` cho chủ tenant. Phát hiện + vá thêm: `FirebasePhoneLoginService` là luồng đăng nhập duy nhất thiếu check tenant suspended. Test live qua API thật: suspend/reactivate đều gửi đúng notification + ghi đúng audit |
| 134 | Xem chi tiết tenant vận hành | ✅ Pass (Playwright thật) | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-19 | Xác nhận "cảnh báo gần vượt limit" đã thỏa mãn ý định AC qua tính toán phía FE (progress bar 80%/100%), không cần vá thêm. Gap thật: storage usage hoàn toàn chưa có hạ tầng theo dõi. **Đã hỏi người dùng, chọn xây nhẹ**: tính dung lượng qua S3/MinIO (avatar + bằng chứng vi phạm), CỐ Ý không gồm ảnh Face ID (hệ thống AI riêng, không phải S3) — số liệu là mức tối thiểu, ghi rõ trong UI. Test live qua API thật: upload 28+26 byte test → API trả đúng tổng byte-for-byte. Test live qua Playwright thật: trang chi tiết tenant hiện đúng "0 / 6" thay vì placeholder cũ, ghi chú giới hạn hiện đúng |
| 135 | Enforce giới hạn gói | — (không có FE thay đổi) | — | ✅ **PASS — ĐÃ KHÓA** | 2026-08-19 | **Phát hiện quan trọng**: nghi ngờ ban đầu là bug enforcement hóa ra là do dữ liệu gói `trial` bị lệch (hệ quả phụ của 1 lần `kill -9` khẩn cấp lúc khôi phục môi trường trước đó) — core enforcement KHÔNG có bug, đã restore dữ liệu. Đã vá đúng gap thật: thêm audit `PLAN_LIMIT_DENIED` khi từ chối tạo employee/site/random-check vượt hạn mức. **Đợt 2 cùng ngày**: vá nốt gap "export chưa kiểm tra limit" — thêm field mới `plan_limits.max_exports_per_month` (migration V107, seed 10/100/1000/unlimited theo tỷ lệ max_random_checks_per_month), đếm usage bằng cách tái dùng audit log `EXPORT_*` sẵn có, enforce ở cả 3 export endpoint (attendance/violations/Face ID not-enrolled). Test live qua API thật: set tạm limit=1 trên gói trial → export lần 1 = 200, lần 2 = 422 `PLAN_LIMIT_EXCEEDED` + đúng dòng `PLAN_LIMIT_DENIED` trong audit_logs; đã restore giá trị gốc sau test. Regression: test_plan_limits.sh (13/13), test_plans.sh (17/17), test_subscription.sh (19/19), test_plan_deactivation_migration.sh (8/8), test_export_attendance.sh (11/11), test_export_violations.sh (10/10) — toàn bộ PASS |
