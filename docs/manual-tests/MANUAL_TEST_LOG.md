...
| 106 | Tạo violation khi không phản hồi | ⬜ Chưa test | — | ⬜ **CHƯA TEST** | — | Kịch bản đã viết. Gap thật: tạo violation `no_response` đúng, idempotent, nhưng KHÔNG gửi notification cho HR/nhân viên (gốc rễ chung với #107, nên vá 1 lần cho cả 2) |
| 107 | Tạo violation khi fail random check | ⬜ Chưa test | — | ⬜ **CHƯA TEST** | — | Kịch bản đã viết. **Cáo buộc nghiêm trọng của audit gốc ("liveness_fail không bao giờ tạo ra") ĐÃ SAI/LỖI THỜI — đã được vá cùng đợt với #104.** AC ghi sai tên type (wrong_location/face_mismatch, thực tế là location_fail/face_fail — chỉ là ghi nhầm tài liệu). 2 gap thật: không gửi notification (chung #106); bằng chứng không nằm sẵn trong violation, phải tra gián tiếp |
| 108 | HR kích hoạt kiểm tra ngay | ⬜ Chưa test | — | ⬜ **CHƯA TEST** | — | Kịch bản đã viết. **Cải chính audit gốc: "không ghi audit" là SAI — ManualCheckService CÓ ghi audit đầy đủ** (chính là ví dụ chuẩn dùng để phát hiện #99 thiếu audit). Gap thật còn lại: trigger_type không phải field rõ ràng, chỉ suy luận ngầm qua checkIndex=0 |
| 109 | HR xem danh sách scheduled checks | ⬜ Chưa test | — | ⬜ **CHƯA TEST** | — | Kịch bản đã viết. **Hạ từ "✅ đã xong" xuống 🟡 — gap MỚI audit gốc bỏ sót:** không lọc được theo trigger_type (hệ quả trực tiếp từ #108, field này không tồn tại). 4/5 filter còn lại (site/employee/status/ngày) đều đúng |
| 110 | HR xem chi tiết random check | ⬜ Chưa test | — | ⬜ **CHƯA TEST** | — | Kịch bản đã viết. Audit gốc "✅ đã xong" về cơ bản đúng, chỉ 1 điểm cần cải chính: "sent_at" không hiển thị được vì field không tồn tại (kế thừa gap #100, vá 1 lần ở #100 sẽ tự khắc phục ở đây) |
