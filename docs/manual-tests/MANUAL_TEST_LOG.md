...
| 111 | HR override check-in | ⬜ Chưa test | — | ⬜ **CHƯA TEST** | — | Kịch bản đã viết. **Gap "không ghi audit" của audit gốc ĐÃ ĐƯỢC VÁ TỪ TRƯỚC** (side-effect của đợt #79 cùng phiên làm việc này) — chỉ cần test live xác nhận lại, không cần sửa thêm |
| 112 | HR chỉnh attendance summary | ⬜ Chưa test | — | ⬜ **CHƯA TEST** | — | Kịch bản đã viết. **Gap "không ghi audit" của audit gốc ĐÃ ĐƯỢC VÁ TỪ TRƯỚC** (side-effect của đợt #84 cùng phiên làm việc này) — chỉ cần test live xác nhận lại, không cần sửa thêm |
| 113 | Nhân viên gửi giải trình check-in lỗi | ⬜ Chưa test | ⬜ Chưa test | ⬜ **CHƯA TEST** | — | Kịch bản đã viết. Gap thật: không gửi notification cho HR. Gap MỚI phát hiện: không liên kết được giải trình với 1 violation cụ thể (AC nhắc cả violations nhưng chỉ có checkin), phạm vi hẹp hơn AC mô tả |
| 114 | HR xem danh sách vi phạm | ⬜ Chưa test | — | ⬜ **CHƯA TEST** | — | Kịch bản đã viết. 2 gap thật: không filter `severity` (field không tồn tại trên entity, cần đổi schema mới vá được) và không filter `affectsAttendance` (field đã có sẵn, chỉ thiếu predicate — dễ vá hơn nhiều) |
| 115 | HR xem chi tiết violation | ⬜ Chưa test | — | ⬜ **CHƯA TEST** | — | Kịch bản đã viết. **Hạ từ "✅ đã xong" xuống 🟡 — 2 gap MỚI audit gốc bỏ sót:** violation từ check-in thường (face_verify_timeout) thiếu bằng chứng ảnh/GPS (chỉ có random-check violation mới đủ); không có ẩn dữ liệu theo quyền như AC yêu cầu |
