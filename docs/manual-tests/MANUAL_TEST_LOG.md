...
| 96 | Tự động sinh scheduled checks đầu ca | — | ⬜ Chưa test | ⬜ **CHƯA TEST** | — | Kịch bản đã viết. KHÔNG có gap thật — AC "random số lần trong min/max" lỗi thời, khớp phát hiện #91 (checksPerShift không có khái niệm min/max). Random thời điểm + đảm bảo min_gap hoạt động đúng, có test script phủ |
| 97 | Snapshot config khi sinh check | — | ⬜ Chưa test | ⬜ **CHƯA TEST** | — | Kịch bản đã viết. Audit gốc "✅ đã xong" xác nhận đúng, không lỗi thời — response validate theo snapshot đã đóng băng lúc sinh lịch, KHÔNG re-fetch config hiện tại (đã xác nhận qua code, không có bug live-refetch) |
| 98 | Tạo job gửi check | — | ⬜ Chưa test | ⬜ **CHƯA TEST** | — | Kịch bản đã viết. Không có gap thật — kiến trúc thật là Redis ZSET + worker poll mỗi phút (không phải BullMQ như tên cũ trong AC), dùng lại UUID của scheduled_check làm khóa, "bull_job_id" trong AC là chi tiết lỗi thời không áp dụng được, không phải thiếu sót |
| 99 | Hủy scheduled check | ⬜ Chưa test | — | ⬜ **CHƯA TEST** | — | Kịch bản đã viết. 2 gap thật: hoàn toàn KHÔNG có field cancelledBy/At/Reason (chưa migration, không phải chỉ quên set), KHÔNG ghi audit log dù pattern đã có sẵn ở chỗ khác cùng module (ManualCheckService). Web Admin đã có UI hủy hoạt động đúng |
| 100 | Gửi random check notification | — | ⬜ Chưa test | ⬜ **CHƯA TEST** | — | Kịch bản đã viết. 2 gap thật: ScheduledCheck không có sent_at lẫn notification_id, phải tra gián tiếp qua notification_delivery_logs/metadata. Luồng gửi + tạo notification + push data payload đều đúng |
| 101 | App hiển thị random check đang chờ | — | ⬜ Chưa test | ⬜ **CHƯA TEST** | — | Kịch bản đã viết. Không có gap — đếm ngược, deep-link, hiển thị theo mode đều đúng thiết kế snapshot |
| 102 | Phản hồi mode chỉ vị trí | — | ⬜ Chưa test | ⬜ **CHƯA TEST** | — | Kịch bản đã viết. Không có gap — pass/fail theo geofence đúng, face fields giữ null đúng AC |
| 103 | Phản hồi mode vị trí + Face ID | — | ⬜ Chưa test | ⬜ **CHƯA TEST** | — | Kịch bản đã viết. Không có gap — nhánh fail ngay (chưa đăng ký Face ID) + nhánh async qua AI worker đều đúng, có idempotency guard chống trùng violation |
| 104 | Phản hồi mode vị trí + Face ID + Liveness | — | ⬜ Chưa test | ⬜ **CHƯA TEST** | — | Kịch bản đã viết. **Cáo buộc nghiêm trọng của audit gốc ("liveness score dead code, không ảnh hưởng pass/fail") ĐÃ SAI/LỖI THỜI — bug này CÓ THẬT lúc audit gốc nhưng đã được vá ở commit sau đó (04-08), xác nhận qua code hiện tại liveness fail đúng đắn làm outcome=fail.** Nuance mới: liveness ở random-check là thụ động (1 ảnh), không có thử thách chủ động như check-in — cần chủ dự án xác nhận mức độ chấp nhận được |
| 105 | Từ chối phản hồi trễ | — | ⬜ Chưa test | ⬜ **CHƯA TEST** | — | Kịch bản đã viết. Không có gap — từ chối đúng, cố ý KHÔNG tự tạo violation (nhiệm vụ đó thuộc #106, tách bạch trách nhiệm đúng thiết kế) |
