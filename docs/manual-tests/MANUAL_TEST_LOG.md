...
| 140 | Retry và fallback notification | — (không có FE thay đổi trực tiếp) | 🟡 Cần bạn test badge "Khẩn cấp" trên thiết bị/app thật | 🟡 **PARTIAL — Backend đã sửa+test, CHƯA commit** | 2026-08-19 | Retry+backoff đã đúng từ trước. **Đã vá gap thật (chưa commit)**: fallback trước đây chạy vô điều kiện cho MỌI loại — nay chỉ chạy khi priority="critical" đúng AC. **Phát hiện phụ, đã hỏi người dùng**: hệ thống trước đó không có eventType nào "critical" — nếu áp đúng AC sẽ tắt hẳn fallback (regression); quyết định nâng `RANDOM_CHECK_SENT` từ "high" lên "critical". Test live API+DB thật: gửi RANDOM_CHECK_SENT tới device giả → có dòng EMAIL_FALLBACK; gửi EMPLOYEE_INVITED (normal) → không có fallback — đúng thiết kế. `test_fcm_retry_fallback.sh` cập nhật, PASS 11/11. SMS fallback: không có hạ tầng, ngoài phạm vi. **⚠️ CẢNH BÁO QUAN TRỌNG**: khi bắt đầu sửa #136-140, working tree đang ở `develop` KHÔNG chứa #131-135 (nhánh đó chưa merge) — rebuild+redeploy Docker trong lúc test đã vô tình làm container đang chạy MẤT tạm các tính năng #131-135 (API hiện không còn field `maxExportsPerMonth`...). Đã hỏi người dùng, **quyết định: để nguyên, tự merge sau** — KHÔNG rebuild/deploy thêm cho tới khi bạn merge nhánh `feature/saved-filters-tenant-ops-plan-limits-131-135` vào `develop`. **CẦN BẠN**: (1) merge nhánh 131-135 vào develop; (2) sau đó rebase/cherry-pick code #140 (hiện nằm uncommitted trên `develop`) lên trên; (3) rebuild+redeploy lại; (4) test badge "Khẩn cấp" trên UI thật. Xem `docs/manual-tests/sprint-6-feature-139-140-notification-templates-fallback.md` |
